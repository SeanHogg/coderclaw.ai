...
read_when:
  ...
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of CoderClaw
...
3. CoderClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. CoderClaw extracts the user identity from the configured header
...
3. Update CoderClaw config with trusted-proxy auth
