...
1. Click [Deploy CoderClaw](https://northflank.com/stacks/deploy-coderclaw) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy CoderClaw now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the CoderClaw template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the CoderClaw service.
8. Open the public CoderClaw URL and complete setup at `/setup`.
9. Open the Control UI at `/coderclaw`.

## What you get

- Hosted CoderClaw Gateway + Control UI
...
4. Click **Run setup**.
5. Open the Control UI at `https://<your-northflank-domain>/coderclaw`
