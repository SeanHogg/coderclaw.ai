...
read_when:
  ...
  - 你想要所有内容都固定并以声明式管理
summary: 使用 Nix 声明式安装 CoderClaw
title: Nix
x-i18n:
...
使用 Nix 运行 CoderClaw 的推荐方式是通过 **[nix-coderclaw](https://github.com/coderclaw/nix-coderclaw)** — 一个开箱即用的 Home Manager 模块。
...
> **📦 完整指南：[github.com/coderclaw/nix-coderclaw](https://github.com/coderclaw/nix-coderclaw)**
>
> nix-coderclaw 仓库是 Nix 安装的权威来源。本页只是一个快速概述。
...
当设置 `CODERCLAW_NIX_MODE=1` 时（nix-coderclaw 会自动设置）：

CoderClaw 支持 **Nix 模式**，使配置确定性并禁用自动安装流程。
通过导出以下环境变量启用：
...
CoderClaw 从 `CODERCLAW_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `CODERCLAW_STATE_DIR` 中。
...
- [nix-coderclaw](https://github.com/coderclaw/nix-coderclaw) — 完整设置指南
