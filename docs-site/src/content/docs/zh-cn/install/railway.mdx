...
通过一键模板在 Railway 上部署 CoderClaw，并在浏览器中完成设置。
...
<a href="https://railway.com/deploy/coderclaw-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/coderclaw` — 控制面板 UI

## 你将获得

- 托管的 CoderClaw Gateway网关 + 控制面板 UI
- `/setup` 网页设置向导（无需终端命令）
...
- `CODERCLAW_STATE_DIR=/data/.coderclaw`（推荐）
- `CODERCLAW_WORKSPACE_DIR=/data/workspace`（推荐）
- `CODERCLAW_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 CoderClaw 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
