---
read_when:
  - 你想了解 `coderclaw.ai/install.sh` 的工作机制
  - 你想自动化安装（CI / 无头环境）
  - 你想从 GitHub 检出安装
...
CoderClaw 提供两个安装器脚本（托管在 `coderclaw.ai`）：

- `https://coderclaw.ai/install.sh` — "推荐"安装器（默认全局 npm 安装；也可从 GitHub 检出安装）
- `https://coderclaw.ai/install-cli.sh` — 无需 root 权限的 CLI 安装器（安装到带有独立 Node 的前缀目录）
- `https://coderclaw.ai/install.ps1` — Windows PowerShell 安装器（默认 npm；可选 git 安装）

查看当前参数/行为，运行：

```bash
curl -fsSL https://coderclaw.ai/install.sh | bash -s -- --help
```

Windows (PowerShell) 帮助：

```powershell
& ([scriptblock]::Create((iwr -useb https://coderclaw.ai/install.ps1))) -?
```

如果安装器完成但在新终端中找不到 `coderclaw`，通常是 Node/npm PATH 问题。参见：[安装](/install#nodejs--npm-path-sanity)。
...
- 选择安装方式：
  - `npm`（默认）：`npm install -g coderclaw@latest`
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL https://coderclaw.ai/install.sh | bash
```

### 可发现性 / "git 安装"提示

如果你在**已有的 CoderClaw 源码检出目录中**运行安装器（通过 `package.json` + `pnpm-workspace.yaml` 检测），它会提示：
...
此脚本将 `coderclaw` 安装到前缀目录（默认：`~/.coderclaw`），同时在该前缀下安装专用的 Node 运行时，因此可以在不想改动系统 Node/npm 的机器上使用。

帮助：

```bash
curl -fsSL https://coderclaw.ai/install-cli.sh | bash -s -- --help
```
...
```powershell
iwr -useb https://coderclaw.ai/install.ps1 | iex -InstallMethod git
```

```powershell
iwr -useb https://coderclaw.ai/install.ps1 | iex -InstallMethod git -GitDir "C:\\coderclaw"
```
