...
summary: CoderClaw macOS 配套应用（菜单栏 + Gateway 网关代理）
title: macOS 应用
x-i18n:
...
# CoderClaw macOS 配套应用（菜单栏 + Gateway 网关代理）

macOS 应用是 CoderClaw 的**菜单栏配套应用**。它拥有权限，在本地管理/附加到 Gateway 网关（launchd 或手动），并作为节点向智能体暴露 macOS 功能。
...
- 根据请求通过 npm/pnpm 安装全局 CLI（`coderclaw`）（不建议使用 bun 作为 Gateway 网关运行时）。
...
应用管理一个标记为 `bot.molt.gateway` 的每用户 LaunchAgent（使用 `--profile`/`CODERCLAW_PROFILE` 时为 `bot.molt.<profile>`；旧版 `com.coderclaw.*` 仍会卸载）。
