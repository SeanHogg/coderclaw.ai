...
- 你仍然可以使用 [Gateway 网关健康](/gateway/health) 中的 CLI 流程（`coderclaw status`、`coderclaw status --deep`、`coderclaw health --json`），并在 `/tmp/coderclaw/coderclaw-*.log` 中跟踪 `web-heartbeat` / `web-reconnect`。
