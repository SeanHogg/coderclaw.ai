...
这与 `~/.coderclaw/` 是分开的，后者存储配置、凭证和会话。
...
当启用沙箱隔离且 `workspaceAccess` 不是 `"rw"` 时，工具在 `~/.coderclaw/sandboxes` 下的沙箱工作区内操作，而不是你的主机工作区。
...
- 在 `~/.coderclaw/coderclaw.json` 中覆盖：
...
如果任何引导文件缺失，CoderClaw 会在会话中注入"缺失文件"标记并继续。大型引导文件在注入时会被截断；使用 `agents.defaults.bootstrapMaxChars` 调整限制（默认：20000）。
`coderclaw setup` 可以重新创建缺失的默认值而不覆盖现有文件。

## 工作区中不包含的内容

这些位于 `~/.coderclaw/` 下，不应提交到工作区仓库：

- `~/.coderclaw/coderclaw.json`（配置）
- `~/.coderclaw/credentials/`（OAuth token、API 密钥）
- `~/.coderclaw/agents/<agentId>/sessions/`（会话记录 + 元数据）
- `~/.coderclaw/skills/`（托管的 Skills）
...
如果你必须存储敏感引用，请使用占位符并将真正的密钥保存在其他地方（密码管理器、环境变量或 `~/.coderclaw/`）。
...
1. 将仓库克隆到所需路径（默认 `~/.coderclaw/workspace`）。
2. 在 `~/.coderclaw/coderclaw.json` 中将 `agents.defaults.workspace` 设置为该路径。
3. 运行 `coderclaw setup --workspace <path>` 来填充任何缺失的文件。
4. 如果你需要会话，请单独从旧机器复制 `~/.coderclaw/agents/<agentId>/sessions/`。
