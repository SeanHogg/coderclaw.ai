...
In CoderClaw, a loop is a single, serialized run per session that emits lifecycle and stream events
...
- System prompt is built from CoderClaw’s base prompt, skills prompt, bootstrap context, and per-run overrides.
