...
All session state is **owned by the gateway** (the “master” CoderClaw). UI clients (macOS app, WebChat, etc.) must query the gateway for session lists and token counts instead of reading local files.
...
- CoderClaw does **not** read legacy Pi/Tau session folders.

## Session pruning

CoderClaw trims **old tool results** from the in-memory context right before LLM calls by default.
This does **not** rewrite JSONL history. See [/concepts/session-pruning](/concepts/session-pruning).

## Pre-compaction memory flush

When a session nears auto-compaction, CoderClaw can run a **silent memory flush**
...
- Legacy idle-only: if you set `session.idleMinutes` without any `session.reset`/`resetByType` config, CoderClaw stays in idle-only mode for backward compatibility.
