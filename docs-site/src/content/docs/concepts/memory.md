---
title: "Memory"
summary: "How CoderClaw memory works (workspace files + automatic memory flush)"
read_when:
...
CoderClaw memory is **plain Markdown in the agent workspace**. The files are the
...
When a session is **close to auto-compaction**, CoderClaw triggers a **silent,
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, CoderClaw auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. CoderClaw
...
- CoderClaw does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - CoderClaw sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index CoderClaw
...
- When `scope` denies a search, CoderClaw logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, CoderClaw exports sanitized session
...
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, CoderClaw automatically resets and reindexes the entire store.
...
- If the sqlite-vec extension is missing or fails to load, CoderClaw logs the
