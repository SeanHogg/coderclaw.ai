---
summary: "Use Synthetic's Anthropic-compatible API in CoderClaw"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. CoderClaw registers it as the
...
Note: CoderClaw's Anthropic client appends `/v1` to the base URL, so use
