...
- CoderClaw installed on your server (Linux flow below tested on Ubuntu 24).
...
4. Configure CoderClaw, restart gateway, verify channel:
...
- **Typing indicators**: CoderClaw sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, CoderClaw forwards read receipts for allowed DMs.
