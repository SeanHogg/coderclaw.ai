...
- Integrations: replace stale Signal docs link with canonical CoderClaw channel docs URL (#44, thanks @deftdawg).
- Docs: rename README references to CoderClaw/coderclaw.ai and update Discord invite branding link (#57, thanks @knocte).
...
- Installer: modernize `install.sh` UX with staged progress, quieter command output, optional gum UI controls (`--gum`, `--no-gum`, `CODERCLAW_USE_GUM`), and verified-only temporary gum bootstrap (#50, thanks @sebslight).
...
- `install.sh`: warn when the user's original shell `PATH` likely won't find the installed `coderclaw` binary (common Node/npm global bin issues); link to docs.
